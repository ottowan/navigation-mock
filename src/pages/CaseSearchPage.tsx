...
export function CaseSearchPage() {
  ...
  if (!storage.hasServiceAccess('case-search')) return <Navigate to="/service/case-search" replace />
  ...
  return <div className="page case-page">
    ...
    <div className="container case-layout">
      <form className="case-form" onSubmit={submit}>
        ...
        <button className="button primary wide" disabled={loading}><Search /> {loading ? 'กำลังค้นหา...' : 'ค้นหา'}</button>
      ...
      <div className="case-result">
        {!result && !loading && <div className="result-placeholder"><Search /><h3>ผลการค้นหาจะแสดงที่นี่</h3><p>กรอกข้อมูลด้านซ้ายแล้วกด “ค้นหา”</p></div>}
